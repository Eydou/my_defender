...
void canon_tower_shoot_assassin(all_t *all, sfRenderWindow *window)
{
    ...
    for (int i = 1; i < all->twr->nbr_canon_twr; i++) {
        pos = sfSprite_getPosition(all->enm->sprite[1]);
        if (pos.x > all->twr->v_canon_tower[i].x - 150 &&
            ...
            if (all->enm->seconds[16] > 2) {
                all->enm->life[1] = all->enm->life[1] - 1;
                sfMusic_play(all->msc->game[4]);
                sfClock_restart(all->enm->clock[16]);
            }
        }
    }
    fire_tower_shoot_assassin(all, window);
}
...
void fire_tower_shoot_assassin(all_t *all, sfRenderWindow *window)
{
    ...
    for (int i = 1; i < all->twr->nbr_fire_twr; i++) {
        ...
        if (pos.x > all->twr->v_fire_tower[i].x - 150 &&
            ...
            if (all->enm->seconds[16] > 2) {
                all->enm->life[1] = all->enm->life[1] - 2;
                sfMusic_play(all->msc->game[5]);
                sfClock_restart(all->enm->clock[16]);
            }
        }
    }
    ice_tower_shoot_assassin(all, window);
}
...
void ice_tower_shoot_assassin(all_t *all, sfRenderWindow *window)
{
    ...
    thunder_tower_shoot_assassin(all, window);
}
...
void thunder_tower_shoot_assassin(all_t *all, sfRenderWindow *window)
{
    ...
    canon_tower_shoot_devil(all, window);
}
